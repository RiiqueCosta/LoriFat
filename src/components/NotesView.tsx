  ...
  return (
    <div className="space-y-6 fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-zinc-900">Bloco de Notas</h2>
          <p className="text-sm text-zinc-500">Anotações rápidas e lembretes do dia a dia.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isAdding && (
          <div className="bg-orange-50 border-2 border-dashed border-orange-200 p-6 rounded-3xl animate-pulse">
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <textarea 
                ...
                className="w-full bg-transparent border-none outline-none text-sm text-zinc-800 placeholder:text-zinc-400 min-h-[100px] resize-none"
              />
        ...
        {filteredNotes.length > 0 ? filteredNotes.map(note => (
          <div 
            key={note.id} 
            className="group relative bg-white p-6 rounded-3xl border border-zinc-200 shadow-sm hover:shadow-md hover:border-brand/30 transition-all flex flex-col h-full"
          >
            ...
            <p className="text-sm text-zinc-800 leading-relaxed flex-1 whitespace-pre-wrap">{note.title}</p>
