    ...
    <div className="space-y-6 fade-in">
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <div className="flex items-center gap-4">
              <div className={cn("p-3 rounded-xl", stat.bg, i === 1 && "dark:bg-green-900/20", i === 2 && "dark:bg-red-900/20")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-zinc-900 dark:text-zinc-100">Evolução de Faturamento</h3>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={document.documentElement.classList.contains('dark') ? '#3F3F46' : '#E2E8F0'} />
                  <XAxis 
                    dataKey="month" 
                    axisLine={false} 
                    tickLine={false} 
                  ...
                  />
                  <Tooltip 
                    cursor={{ fill: document.documentElement.classList.contains('dark') ? '#27272A' : '#F8FAFC' }}
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', backgroundColor: document.documentElement.classList.contains('dark') ? '#18181B' : '#FFFFFF', color: document.documentElement.classList.contains('dark') ? '#F4F4F5' : '#18181B' }}
              ...
              <div className="h-full flex items-center justify-center text-zinc-400 dark:text-zinc-600 text-sm italic">
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col">
          <h3 className="font-bold text-zinc-900 dark:text-zinc-100 mb-6">Status Financeiro</h3>
          <div className="flex-1 flex flex-col justify-center items-center space-y-6">
            <div className="relative">
              <svg className="w-32 h-32 transform -rotate-90">
                <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" className="text-zinc-100 dark:text-zinc-800" />
                <circle 
                  cx="64" 
                  cy="64" 
                  r="58" 
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{totalInvoiced > 0 ? Math.round((totalPaid/totalInvoiced)*100) : 0}%</p>
                <p className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase">Recebido</p>
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
          <h3 className="font-bold text-zinc-900 dark:text-zinc-100">Atividades Recentes</h3>
        ...
        <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {invoices.length > 0 ? invoices.slice(-5).reverse().map((inv) => (
            <div key={inv.id} className="p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800 flex items-center justify-between transition-colors">
              <div className="flex items-center gap-4">
                <div className={cn("p-2 rounded-lg", inv.status === 'paid' ? "bg-green-100 dark:bg-green-900/20 text-green-600" : "bg-yellow-100 dark:bg-yellow-900/20 text-yellow-600")}>
                  <FileText className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{inv.clientName}</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">{inv.description}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{formatCurrency(inv.total)}</p>
                <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-medium">{formatDate(inv.dateCreated)}</p>
              </div>
            </div>
          )) : (
