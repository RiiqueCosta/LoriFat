...
export function Sidebar({ currentView, onViewChange, isOpen, onClose }: SidebarProps) {
  return (
    <>
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transition-transform duration-300 lg:translate-x-0 lg:static",
        ...
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-zinc-900 dark:text-zinc-100 leading-tight">Lori-TI</h1>
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest font-semibold">Soluções Tech</p>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("w-5 h-5", isActive ? "text-white" : "text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-200")} />
                  {item.label}
                </button>
              );
            })}
